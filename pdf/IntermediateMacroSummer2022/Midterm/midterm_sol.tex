...
First of all, in social planner's problem, all markets must clear, and thus $ N^{s} = N^{d} = N $, and $ K^{s} = K^{d} = K $ (also $ K = 2 $, but for question \ref{wagealgebra} to \ref{K}, still remain the symbol $ K $).
...
\Question $ \displaystyle U(C(l), l) =  $ $ \frac{\textrm{~\answer{$(z 2^{a}(1-l)^{1-a} - G)$}}^{1-b}}{1-b} + \frac{l^{1-d}}{1-d}$

Let $ z = 1, G = 0, a = \frac{1}{2}, b = 2, d = \frac{3}{2}$ and solve for all unknowns,

\Question \label{lnumeric} $ l =  $ ~\answer{$0.72$}
\Question \label{Nnumeric} $ N =  $ ~\answer{$0.28$}
\Question \label{wnumeric} $ w^{*} =  $ ~\answer{$2.67$}
\Question \label{rnumeric} $ r^{*} =  $ ~\answer{$0.19$}
