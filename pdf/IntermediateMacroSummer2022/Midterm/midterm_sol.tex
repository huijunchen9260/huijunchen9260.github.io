...
Consider a model economy with distorting labor taxes similar to Lecture 11, but having two difference: \begin{enumerate} \item the production function is Cobb-Douglas requiring only labor input, i.e., $ Y = z N^{a} $, and \item consumer get ``disutility'' from working, and the utility function is given by $ U(C, N^{s}) = \ln C - bN $, where $ b $ is a parameter. \end{enumerate} Other than the two changes mentioned above, the definition of the general equilibrium is exactly the same as slide 5 in Lecture 11. Therefore, this question focus mainly on algebraic calculation.

\Question The derivative of utility function with respect to consumption $ C $ is ~\answer{ $\frac{1}{C}$}

\Question The derivative of utility function with respect to leisure $ l $ is ~\answer{ $b$}

\Question \label{MRS} The marginal rate of substitution between leisure and consumption ($MRS_{l, C}$) is ~\answer{ $b C$}

\Question \label{eq} According to the slide 6 and 9, Lecture 11, in equilibrium MRS is going to be equal to ~\answer{$ w (1-t) $}

\Question \label{budget} According to the slide 5, Lecture 11, consumer's budget constraint is saying that $ C =  $ ~\answer{ $ w(1-t) $} $ N^{s} + \pi $

\Question \label{wN} Different from slide 5, Lecture 11, now the production function is $ Y = z N^{a} $, and thus by solving the firm's problem, the equilibrium wage as a function of labor demand $ N $ must be $ w =  $ ~\answer{$a z N^{a-1}$}

\Question \label{pi} Following \ref{wN}, firm's profit $ \pi = $ ~\answer{$(1-a) z N^{a}$}

\Question \label{sol1} Combining your answer in \ref{MRS} and \ref{eq} together, substitute consumption from your answer in \ref{budget}, and imposing labor market clearing condition so that $ N^{s} = N $, we get the optimal condition is $ b ($~\answer{$(w (1-t) N) + \pi$} $) = w(1-t) $
...
Combining your answer in \ref{wN} and \ref{pi} and substitute $ w $ and $ \pi $ into your answer in \ref{sol1}, we get
    ...
    $ b(  $
    \label{wpart1} $ \underbrace{ \textrm{~\answer{ $ azN^{a} (1-t)$}} }_{w \textrm{ part}}$
    $+$
    \label{pipart} $ \underbrace{ \textrm{~\answer{$(1-a) z N^{a} $}} }_{\pi \textrm{ part}}$
    $) =$
    \label{wpart2} $ \underbrace{ \textrm{~\answer{ $ azN^{a-1} $}} }_{w \textrm{ part 2}}$
    $  (1-t)  $

    \Question \label{wpart1} $ \underbrace{ \textrm{~\answer{ $ azN^{a} (1-t)$}} }_{w \textrm{ part}}$
    \Question \label{pipart} $ \underbrace{ \textrm{~\answer{$(1-a) z N^{a} $}} }_{\pi \textrm{ part}}$
    \Question \label{wpart2} $ \underbrace{ \textrm{~\answer{ $ azN^{a-1} $}} }_{w \textrm{ part 2}}$

\Question \label{sol3} Solve for $ N $ as a function of $ t $, we get
    $ N =  $ ~\answer{ $ \frac{a (1-t)}{b (a (1-t) + (1-a))} $ }
